...
const ExpertosPage = () => {
  ...
  const [semblanzaText, setSemblanzaText] = useState("");
      ...
      if (status === "authenticated") {
        try {
          ...
          if (data.success && data.data) {
            ...
            // Inicializar semblanza
            if (data.data.semblanza) {
              setSemblanzaText(data.data.semblanza);
              const words = data.data.semblanza.trim().split(/\s+/).filter(word => word.length > 0);
  ...
  useEffect(() => {
    const fetchExpertosData = async () => {
      try {
        ...
        // Solo usar datos mock si no hay datos reales del experto
        if (!experto) {
          ...
          if (data.expertos_formulario && data.expertos_formulario.length > 0) {
            ...
            // Inicializar semblanza con datos mock
            setSemblanzaText(perfilPedro.semblanza);
            const words = perfilPedro.semblanza.trim().split(/\s+/).filter(word => word.length > 0);
            setWordCount(words.length);
          }
        }
      } catch (error) {
        console.error('Error al cargar datos de expertos:', error);
        // Solo usar datos mock de respaldo si no hay datos reales
        if (!experto) {
          const perfilPedro = {
            _id: "pedro_001",
            nombre: "Pedro García",
          ...
          setExperto(perfilPedro);
          // Inicializar semblanza con datos de respaldo
          setSemblanzaText(perfilPedro.semblanza);
          const words = perfilPedro.semblanza.trim().split(/\s+/).filter(word => word.length > 0);
          setWordCount(words.length);
          setExpertosData({
  ...
  const handleSemblanzaChange = (e) => {
    const text = e.target.value;
    setSemblanzaText(text);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
          ...
          <div className="space-y-6">
             ...
             <div className="bg-white rounded-lg shadow p-4">
               <div className="flex justify-center mb-4">
                 <img 
                   src={session?.user?.image || "https://via.placeholder.com/200x200"} 
                   alt="Foto de perfil" 
        ...
        <main className="flex-1 w-full">
          ...
          <div className="lg:hidden flex flex-col gap-4 mb-8">
                         {/* Foto de Google para móvil */}
             <div className="bg-white rounded-lg shadow p-4">
               <div className="flex justify-center mb-4">
                 <img 
                   src={session?.user?.image || "https://via.placeholder.com/150x150"} 
                   alt="Foto de perfil" 
               ...
               {experto?.nombre || session.user.name || "Nombre del Experto"}
                           ...
                           {/* Estrellas grises pegadas al nombre */}
              ...
              {/* Título de Categorías y Botón Hacer análisis en la misma línea */}
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Categorías</h2>
                <button
                  className="bg-[#1A3D7C] text-white px-4 py-2 rounded-md hover:bg-[#0f2a5a] transition-colors shadow text-sm"
                >
                  Hacer análisis
                </button>
              </div>
              <div className="bg-gray-100 rounded-lg p-4 mb-6 min-h-[60px]">
                <div className="text-gray-400 text-sm">(Espacio para categorías)</div>
              </div>

              {/* Semblanza editable */}
              <div className="mb-6">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-xl font-semibold text-blue-800">Semblanza</h3>
                  <button
                    className="bg-[#1A3D7C] text-white p-2 rounded-full hover:bg-[#0f2a5a] transition-colors shadow"
                  >
                ...
                <textarea
                  placeholder="Escribe tu semblanza profesional aquí (máximo 120 palabras)..."
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1A3D7C] focus:border-[#1A3D7C] transition-colors resize-none text-base"
                  rows={6}
                  value={semblanzaText}
                  onChange={handleSemblanzaChange}
                />
                <div className="text-xs text-gray-500 mt-2 text-right">
                  {wordCount}/120 palabras
                </div>
              </div>

            {/* Dos tarjetas: Servicios sugeridos y Habilidades */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {/* Tarjeta izquierda: Servicios sugeridos */}
              <div className="bg-white rounded-lg shadow p-4">
                <h3 className="font-semibold text-base mb-2">Servicios sugeridos</h3>
                <div className="text-gray-400 text-sm">(Próximamente)</div>
              </div>
              
              {/* Tarjeta derecha: Habilidades que el cliente busca */}
              <div className="bg-white rounded-lg shadow p-4">
                <h3 className="font-semibold text-base mb-2">Habilidades que el cliente busca</h3>
                <div className="text-gray-400 text-sm">(Próximamente)</div>
              </div>
            </div>
          ...
          <div className="hidden lg:block">
              ...
              <div className="bg-gray-100 rounded-lg p-6 mb-8 min-h-[80px]">
                <div className="text-gray-400 text-sm">(Espacio para categorías)</div>
              </div>
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-2xl font-semibold text-blue-800">Semblanza</h3>
                  <button
                    className="bg-[#1A3D7C] text-white p-2 rounded-full hover:bg-[#0f2a5a] transition-colors shadow"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
